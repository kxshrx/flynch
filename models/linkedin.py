...
class LinkedInProfile(LinkedInBase):
    ...
    # Relationships
    experiences = relationship("LinkedInExperience", back_populates="profile", cascade="all, delete-orphan")
    educations = relationship("LinkedInEducation", back_populates="profile", cascade="all, delete-orphan")
...
class LinkedInExperience(LinkedInBase):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    profile_id = Column(String, ForeignKey("linkedin_profiles.id"), nullable=False, index=True)
